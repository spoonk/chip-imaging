    ...
    def get_manual_grid(self, h, w):
        ...
        try:
            grid = load_grid_from_json(
                os.path.join(data_path, GRID_PROPERTIES_FILE_NAME)
            )
            images = (
                self._stitcher._load_tiff_images()
            )  # TODO: stitcher shouldn't be responsible for loading iamges, that should be done here
            ...
            for r in range(h):
                ...
                for c in range(w):
                    ...
                    image_grid_row.append(
                        encodebytes(img_byte_arr.getvalue()).decode("ascii")
                    )
...
def check_stitchable_dir(dir_path: str) -> bool:
    ...
    for item in os.listdir(dir_path):
        ...
        ):  # must check absolute path to file
